...
import (
	...
	headerCleanup "storxscan/blockchain/cleanup"
	"storxscan/health"
	"storxscan/tokenprice"
	tokenPriceCleanup "storxscan/tokenprice/cleanup"
...
// Config wraps storxscan configuration.
type Config struct {
	Debug             debug.Config
...
// DB is a collection of storxscan databases.
type DB interface {
	// Headers creates headers database methods.
	Headers() blockchain.HeadersDB
	// TokenPrice returns database for STORX token price information.
...
// App is the storxscan process that runs API endpoint.
//
// architecture: Peer
type App struct {
	Log      *zap.Logger
...
// NewApp creates new storxscan application instance.
...
// Run runs storxscan until it's either closed or it errors.
