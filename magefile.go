...
func Format() error {
	...
	return sh.RunV("goimports", "-w", "-local=storx", ".")
}
...
// DockerBuild builds storxscan image.
//
//nolint:deadcode
func DockerBuild() error {
	tag, err := getNextDockerTag("storxscan.last")
	if err != nil {
		return err
	}
	err = sh.RunV("docker", "build", "-t", "img.dev.storx.io/storxup/storxscan:"+tag, ".")
	if err != nil {
		return err
	}
	return nil
...
// DockerPublish pushes storxscan image.
//
//nolint:deadcode
func DockerPublish() error {
	return dockerPushWithNextTag("storxscan")
}
...
// ListImages prints all the existing storxscan images in the repo.
//
//nolint:deadcode
func ListImages() error {
	versions, err := listContainerVersions("storxscan")
	if err != nil {
		return err
	}
	for _, v := range versions {
		fmt.Printf("storxscan:%s\n", v)
	}
	return nil
}
...
func dockerPushWithNextTag(image string) error {
	...
	err = sh.RunV("docker", "push", fmt.Sprintf("img.dev.storx.io/storxup/%s:%s", image, tag))
	if err != nil {
		return err
	}
...
func ListVersions() error {
	fmt.Println("container: storxscan")
	versions, err := listContainerVersions("storxscan")
	if err != nil {
		return err
	}
...
func listReleaseVersions(name string) ([]string, error) {
	url := fmt.Sprintf("https://api.github.com/repos/storxscan/%s/releases?per_page=10", name)
...
func listContainerVersions(name string) ([]string, error) {
	...
	url := fmt.Sprintf("https://img.dev.storx.io/auth?service=img.dev.storx.io&scope=repository:%s:pull", name)
	...
	url = fmt.Sprintf("https://img.dev.storx.io/v2/storxup/%s/tags/list", name)
