...
const (
	// storxID is the permanent CoinMarketCap ID associated with STORX token.
	storxID = "1772"
...
// Client is used to query the coinmarketcap API for the STORX token price.
...
func (c *Client) GetLatestPrice(ctx context.Context) (time.Time, currency.Amount, error) {
	...
	timestamp, err := time.Parse(time.RFC3339Nano, formattedResp.Data[storxID].Quote[usdSymbol].LastUpdated)
	if err != nil {
		return time.Time{}, currency.Amount{}, ErrClient.Wrap(err)
	}

	amount := currency.AmountFromDecimal(formattedResp.Data[storxID].Quote[usdSymbol].Price, currency.USDollarsMicro)
