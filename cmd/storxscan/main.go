...
var (
	rootCmd = &cobra.Command{
		Use:   "storxscan",
		Short: "STORX token payment management service",
	}
...
func run(ctx context.Context, config runConfig) error {
	...
	log := logger.Named("storxscan")

	if err := process.InitMetricsWithHostname(ctx, log, nil); err != nil {
		log.Warn("Failed to initialize telemetry batcher for storxscan", zap.Error(err))
	}
	...
	app, err := storxscan.NewApp(log, config.Config, db)
	if err != nil {
		return err
	}
...
func openDatabaseWithRetry(ctx context.Context, logger *zap.Logger, databaseURL string) (db *storxscandb.DB, err error) {
	for i := 0; i < 120; i++ {
		db, err = storxscandb.Open(ctx, logger.Named("storxscandb"), databaseURL)
		if err == nil {
			break
		}
