...
var (
	...
	// Error is the default storxscandb errs class.
	Error = errs.Class("storxscandb")
)

// DB is storxscan database.
type DB struct {
	*dbx.DB
	log            *zap.Logger
	driver         string
...
// Open creates instance of storxscan DB.
func Open(ctx context.Context, log *zap.Logger, databaseURL string) (*DB, error) {
	...
	source, err = pgutil.CheckApplicationName(source, "storxscan")
	if err != nil {
		return nil, err
	}
	...
	dbutil.Configure(ctx, dbxDB.DB, "storxscandb", mon)
