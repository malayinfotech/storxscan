    ...
    stages {
        ...
        stage('Verification') {
            parallel {
                ...
                stage('Tests') {
                    environment {
                        STORX_TEST_POSTGRES = 'postgres://postgres@localhost/teststorxscan?sslmode=disable'
                        STORX_TEST_COCKROACH = 'cockroach://root@localhost:26257/testcockroach?sslmode=disable'
                        COVERFLAGS = "${ env.BRANCH_NAME == 'main' ? '-coverprofile=.build/coverprofile -coverpkg=storx.io/storxscan/...' : ''}"
                    }
                    steps {
                        sh 'psql -U postgres -c \'create database teststorxscan;\''
