...
type Config struct {
	Endpoint string `help:"Ethereum RPC endpoint" devDefault:"http://localhost:8545" releaseDefault:"/home/storx/.ethereum/geth.ipc"`
	Contract string `help:"Address of the STORX token to scan for transactions" default:"0xb64ef51c888972c908cfacf59b47c1afbc0ab8ac"`
}
...
func paymentFromEvent(event *erc20.ERC20Transfer, timestamp time.Time, price currency.Amount) Payment {
	tokenValue := currency.AmountFromBaseUnits(event.Value.Int64(), currency.StorxToken)
	return Payment{
		From:        event.From,
		To:          event.To,
