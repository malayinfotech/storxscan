...
func testPayments(t *testing.T, connStr string) {
	testeth.Run(t, func(ctx *testcontext.Context, t *testing.T, tokenAddress common.Address, network *testeth.Network) {
		...
		db, err := storxscandbtest.OpenDB(ctx, zaptest.NewLogger(t), connStr, t.Name(), "T")
		if err != nil {
			t.Fatal(err)
		}
		...
		for i, payment := range payments {
			...
			a := currency.AmountFromBaseUnits(testPayment.Amount, currency.StorxToken)
...
func testAllPayments(t *testing.T, connStr string) {
	testeth.Run(t, func(ctx *testcontext.Context, t *testing.T, tokenAddress common.Address, network *testeth.Network) {
		...
		t.Run("eu1 from block 0", func(t *testing.T) {
			...
			a1 := currency.AmountFromBaseUnits(testPayments[1].Amount, currency.StorxToken)
			a2 := currency.AmountFromBaseUnits(testPayments[2].Amount, currency.StorxToken)
			a4 := currency.AmountFromBaseUnits(testPayments[4].Amount, currency.StorxToken)
			a5 := currency.AmountFromBaseUnits(testPayments[5].Amount, currency.StorxToken)
